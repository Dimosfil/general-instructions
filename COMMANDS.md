...
Task-manager config is project-local state. `base_url` is the manager API
endpoint for operations, not a UI URL unless the adapter explicitly says one
origin serves both UI and API. Before posting plans or starting sprint work,
verify the required workflow capability, not only generic health. If health
succeeds but required endpoints fail, stop and report a stale or misconfigured
manager endpoint.
...
Before starting sprint workflow, verify that the configured manager API
endpoint supports active sprint lookup, next-task lookup, and task completion
for the selected workflow. If only generic health works, stop before executing
tasks.
...
`base_url` must be the selected manager's API endpoint for the current project.
Do not use a UI URL unless the adapter explicitly says the UI URL is also the
API URL. Do not copy an endpoint from another project without user
confirmation. After setup, verify the capabilities needed for the next
workflow.
...
Before sending, verify raw intake capability, such as the contract or raw
endpoint, not only `/health`. If `/health` succeeds but raw intake is
unavailable, stop and report a stale or misconfigured manager endpoint.
