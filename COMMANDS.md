...
```text
gi обновись
gi init .\general-instructions\
инит .\general-instructions\
init .\general-instructions\
gi язык
ги язык
gi язык: 2 1
ги язык: 2 1
...
задан локальными инструкциями, затем `config/gi-main.json` из текущей shared
instruction library, относительного пути shared library или пути из
`GENERAL_INSTRUCTIONS_HOME`. Из GI main config взять `configServiceUrl` и
проверить сам config-service через его `/health` или документированный
discovery endpoint.
...
Если сохранённый shared library path недоступен — использовать относительный
путь из команды, `GENERAL_INSTRUCTIONS_HOME`, текущую shared library, если
команда выполняется внутри неё, или спросить.
